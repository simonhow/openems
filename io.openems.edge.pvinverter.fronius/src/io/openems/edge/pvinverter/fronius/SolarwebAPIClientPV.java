...
public class SolarwebAPIClientPV {
...
private static final String BASE_URL = "https://api.solarweb.com/swqapi/pvsystems/";
	...
	public SolarwebAPIClientPV(String accessKeyID, String accessKeyValue, String pvSystemID) {
	...
	public int getPowerPV() throws OpenemsException {
		/*
		 *  Parse Json object
		 */
		...
		/*
		 * Find channel with name "PowerPV" in Json-Array, 
		...
		do {
			channelName = jsonResponse.getAsJsonObject("data")
					...
					.getAsString();
			channelIndex ++;
		}while (!channelName.equals("PowerPV") && channelIndex < jsonResponse.getAsJsonObject("data").getAsJsonArray("channels").size());
		/*
		 * Read channel with name "PowerPV" in Json-Array
		 */
		if(channelName.equals("PowerPV")) {
		 ...
		 */
			...
			throw new OpenemsException("Could not find channel \"PowerPV\" in JSON object!");
		}
	}
	
	public int getMaxPowerPV() throws OpenemsNamedException {
		String endpoint = this.pvSystemID;
		String response = this.sendGetRequest(endpoint);
		...
		/*
		 * Read channel with name "peakPower" in Json-Array
		 */
		JsonPrimitive peakPowerPrimitive = jsonResponse.getAsJsonPrimitive("peakPower");
		String peakPower = peakPowerPrimitive.getAsString();
		
		return (int) Float.parseFloat(peakPower);
	}
